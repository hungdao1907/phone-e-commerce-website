...
export function TabletFinalCTASection({ config }: { config: TabletBrandConfig }) {
  const reducedMotion = useReducedMotion();
  const scrollToProducts = () => document.getElementById('tablet-all-products')?.scrollIntoView({ behavior: 'smooth' });
  ...
  return (
    <section aria-label={'Khám phá ' + config.label} className="overflow-hidden bg-white px-4 py-24 sm:px-6 sm:py-28 lg:px-8">
      <div className="relative mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-10 overflow-hidden rounded-[2rem] bg-[#f6f7f9] px-6 py-12 sm:px-10 lg:flex-row lg:px-16">
        <div className="tablet-final__glow" aria-hidden="true" />
        <motion.div
        ...
        >
          ...
          <button type="button" onClick={scrollToProducts} className="mt-8 inline-flex min-h-11 items-center gap-2 rounded-full bg-neutral-950 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-black">
        ...
        <motion.img
          ...
          alt={config.label + ' prototype tablet visual'}
          loading="lazy"
