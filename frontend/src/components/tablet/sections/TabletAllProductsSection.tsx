...
interface TabletAllProductsSectionProps {
  ...
  products: TabletModel[]; // Kept for compatibility
}
...
export function TabletAllProductsSection({ config }: TabletAllProductsSectionProps) {
  const containerRef = useRef<HTMLElement>(null);
  const [searchParams, setSearchParams] = useSearchParams();
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);

  const categorySlug = 'tablet';
  let brandSlug = config.id.toLowerCase();
  if (brandSlug === 'ipad') brandSlug = 'apple';
  else if (brandSlug === 'samsung-tablet') brandSlug = 'samsung';
  else if (brandSlug === 'xiaomi-tablet') brandSlug = 'xiaomi';
              ...
              return (
                <button
                  ...
                  className={
                    ...
                    (isActive
                      ? 'border-[var(--tablet-accent)] bg-white text-[var(--tablet-accent)] shadow-[0_4px_14px_rgba(15,23,42,0.08)] font-bold'
          ...
          `}>
            ...
            `}>
              ...
              <div className="md:hidden sticky bottom-0 p-4 bg-white border-t border-neutral-200">
                <button onClick={() => setIsMobileSidebarOpen(false)} className="w-full bg-[var(--tablet-accent)] text-white font-bold py-3 rounded-xl">
                  Xem kết quả
                </button>
              </div>
            </div>
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-full text-sm font-bold flex items-center justify-center transition-colors ${
                            productsData.page === idx + 1 
                              ? 'bg-[var(--tablet-accent)] text-white' 
