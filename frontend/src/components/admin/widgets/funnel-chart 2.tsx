...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-white/90 text-[10px]">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-full bg-white/10 px-2 py-0.5 font-bold text-white text-[9px] shadow-sm backdrop-blur-md">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-medium text-white/60 text-[9px]">
      {stage.label}
    </span>
  );
