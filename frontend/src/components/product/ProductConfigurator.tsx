          ...
          {product.colors.map((color) => {
            ...
            return (
              <button
                ...
                aria-pressed={isSelected}
                className={'flex flex-col items-center justify-center p-2 rounded-xl border transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ' + (
                  isSelected
                    ? 'border-red-500 bg-red-50 text-red-700'
                    : 'border-neutral-200 bg-white hover:border-neutral-400 text-neutral-700'
                )}
              >
                <span className="h-6 w-6 rounded-full border border-black/10 shadow-sm mb-1.5" style={{ backgroundColor: color.hex }} />
                <span className="text-[11px] font-medium text-center line-clamp-1">{color.name}</span>
              </button>
            );
          })}
        </div>
