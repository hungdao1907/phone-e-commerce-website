...
import type { WatchBrandModel } from '@/types/watch/brand/types/index';

interface WatchBrandProductCardProps {
  key?: string | number;
  product: WatchBrandModel;
  priority?: boolean;
  index?: number;
}

const formatWatchBrandPrice = (value: number) => new Intl.NumberFormat('vi-VN', {
  style: 'currency',
  currency: 'VND',
  maximumFractionDigits: 0,
}).format(value);

export function WatchBrandProductCard({ product, priority = false, index = 0 }: WatchBrandProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="watch-product-card group relative flex flex-col h-full bg-white rounded-3xl border border-neutral-200/80 hover:border-neutral-300/90 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_35px_-10px_rgba(0,0,0,0.1)] hover:-translate-y-1.5 transition-all duration-300 overflow-hidden"
    >
      {/* Top Badge & Series */}
      ...
      <div className="relative w-full aspect-[4/3] px-6 py-2 flex items-center justify-center overflow-hidden bg-white">
        ...
        <div
          ...
          style={{ background: `radial-gradient(circle at 50% 50%, var(--watch-accent, #6366f1), transparent 70%)` }}
        />
      ...
      <div className="flex flex-col flex-grow p-6 pt-2">
        ...
        <div className="mb-4">
          <h4
            className="text-xl font-bold text-neutral-900 tracking-tight transition-colors duration-200 hover:text-[var(--watch-accent)]"
          >
            {product.name}
          </h4>
        </div>
        ...
        <div className="mt-auto pt-2 flex items-center justify-between gap-3">
          <div className="flex flex-col">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-base font-bold text-neutral-900 tracking-tight">
                {formatWatchBrandPrice(product.price)}
              </span>
            </div>
            {product.originalPrice && (
              <span className="text-[11px] text-neutral-400 line-through">
                {formatWatchBrandPrice(product.originalPrice)}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isAdding}
              className={`inline-flex items-center justify-center w-8 h-8 rounded-full ${isAdding ? 'bg-neutral-200 text-neutral-400' : 'bg-neutral-100 hover:bg-[var(--watch-accent-soft)] text-neutral-600 hover:text-[var(--watch-accent)]'} transition-colors duration-200 cursor-pointer shrink-0`}
