...
// Exact StarBackground SVG from 21st.dev (@aghasisahakyan1/star-button)
function StarBackground({ color = 'currentColor' }: { color?: string }) {
  return (
    <svg
      width="100%"
      height="100%"
...
export const StarButton = React.forwardRef<HTMLButtonElement, StarButtonProps>(
  (
    ...
    // Handle variant defaults
    const resolvedLightColor =
      lightColor || (variant === 'primary' ? '#2997ff' : '#FAFAFA');
    const resolvedBgColor =
      backgroundColor ||
      (variant === 'primary' ? 'rgba(0, 113, 227, 0.85)' : 'rgba(10, 14, 26, 0.92)');
    ...
    return (
      <button
        ...
        className={`relative z-[3] overflow-hidden px-8 py-3.5 inline-flex items-center justify-center gap-2.5 whitespace-nowrap rounded-full text-sm sm:text-base font-semibold transition-all duration-300 disabled:pointer-events-none disabled:opacity-50 group/star-button cursor-pointer ${className}`}
        {...props}
      >
        {/* Radial Light Animation along Button Border */}
        <div
        ...
        {/* Starfield Background Layer with Inner Border */}
        <div
          className="absolute inset-0 border border-white/25 z-[4] overflow-hidden rounded-[inherit] pointer-events-none backdrop-blur-md"
          style={{ borderWidth: 'var(--border-width)' }}
          aria-hidden="true"
        >
          <StarBackground color={resolvedBgColor} />
        </div>

        {/* Button Content */}
        <span className="z-10 relative inline-flex items-center justify-center gap-2 font-semibold">
          {children}
        </span>
      </button>
    );
