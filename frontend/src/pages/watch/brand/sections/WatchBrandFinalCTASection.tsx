...
export function WatchBrandFinalCTASection({ config }: { config: WatchBrandConfig }) {
  const reducedMotion = useReducedMotion();
  const scrollToProducts = () => document.getElementById('watch-all-products')?.scrollIntoView({ behavior: 'smooth' });
  ...
  return (
    <section aria-label={'Khám phá ' + config.label} className="overflow-hidden bg-white px-4 py-24 sm:px-6 sm:py-28 lg:px-8">
      <div className="relative mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-10 overflow-hidden rounded-[2rem] bg-[#f6f7f9] px-6 py-12 sm:px-10 lg:flex-row lg:px-16">
        <div className="watch-final__glow" aria-hidden="true" />
        <motion.div
        ...
        <motion.img
          ...
          alt={config.label + ' prototype watch visual'}
          loading="lazy"
